...
class GoldiloxAPIClientConnection():
    ...
    def assert_buffer_is_file(self, buffer: bytearray) -> bool:
        ...
        for index, character in enumerate(buffer):
            ...
            if character == '\x44':
                ...
                print('flag checker ', flag_checker)
                if flag_checker == '\x44\x33\x22':
                    return True
    ...
    def FileSender(self, filename: str) -> None:
        ...
        buffer = str(filename) + '\x44\x33\x22\x11' + str(file_content)

        buffer_size = len(filename) + 4 + file_size

        self.sockfd.send(str(buffer_size).encode('utf-8'))
        progress = self.sockfd.recv(400)
